...
@app.route('/transactions/recurring', methods=['GET'])
def get_recurring_transactions():
  ...
  try:
    ...
    for i in range(0, len(transactions)):
        # Discard the credited amounts, focus only on debits
        if transactions[i]['amount'] < 0:
            continue

        name_one = transactions[i]['name'].lower()
        matching_names = [name_one]
        for j in range(i+1, len(transactions)):
            # Discard the credited amounts, focus only on debits
            if transactions[j]['amount'] < 0:
                continue

            # Check if the merchant names are similar
            name_two = transactions[j]['name'].lower()
            if SequenceMatcher(None, name_one, name_two).ratio() > 0.6:
                matching_names.append(name_two)

        # If recurring merchants found, isolate the common prefix for the names
        if len(matching_names) > 1:
            name_to_add = os.path.commonprefix(matching_names)
            if name_to_add not in recurring_transactions:
                recurring_transactions.append(name_to_add)

  except plaid.errors.PlaidError as e:
    return jsonify(format_error(e))
  pretty_print_response(recurring_transactions)
  return jsonify({'error': None, 'recurring_transactions': recurring_transactions})
